defmodule Membrane.MPEGTS.MuxerFilter do
  @moduledoc """
  ...
  def handle_pad_added(pad, %{playback: :stopped}, state) do
    ...
    {binary, muxer} = Muxer.register_track(get_track_type(pad), state.muxer)
  ...
  defp process_popped_items(items, muxer) do
    Enum.flat_map_reduce(items, muxer, fn
      ...
      {pad, {:buffer, buffer}}, muxer ->
        ...
        {payload, muxer} =
          Muxer.put_frame(buffer.payload, get_track_type(pad), pts_ms, dts_ms, muxer)
